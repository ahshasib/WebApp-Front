...
const Withdrawal = () => {
  ...
  const handleSubmit = async () => {
    if (!amount || !fundPassword) {
      Swal.fire("Error", "Please select amount and enter fund password", "error");
      return;
    }

    try {
      const res = await axios.post("http://localhost:3000/withdrawal/request", {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-xl p-4">
          <p className="text-md text-gray-600 mb-3">Withdrawal amount</p>
            ...
            {amounts.map((item, index) => (
              <button
                ...
                className={`border border-gray-200 rounded-lg py-2 text-sm ${
                  amount === item ? "bg-blue-500 text-white" : "bg-gray-50"
                }`}
              >
                {item}
              </button>
        ...
        {/* Submit Button */}
        <button onClick={handleSubmit} className="w-full bg-sky-500 text-white py-3 rounded">
          Submit
        </button>
      </div>
    </div>
